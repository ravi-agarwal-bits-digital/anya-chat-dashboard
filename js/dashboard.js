...
function transcriptHtml(r,includeSummary=true){
  const turns=splitTurns(r.conv);
  let h=includeSummary?`<div style="font-size:12px;color:var(--muted);margin-bottom:10px">${esc(r.summary||'No summary.')}</div>`:'';
  h+=`<div class="transcript">`;
  if(!turns.length)h+=`<div class="empty">Empty conversation.</div>`;
  turns.forEach(t=>{const isUser=t.role==='User';h+=`<div class="t-turn ${isUser?'t-turn-user':'t-turn-agent'}"><span class="t-role">${isUser?'Prospect':'Anya'}</span><span class="t-message ${isUser?'t-user':'t-agent'}">${esc(t.text)}</span></div>`;});
  return h+`</div>`;
}
